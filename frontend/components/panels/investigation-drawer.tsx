    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/55">
      <div className="h-full w-full max-w-2xl overflow-y-auto border-l border-slate-700 bg-slate-950 p-6 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-100">Alert Investigation</h2>
          <button
            type="button"
            onClick={onClose}
              ...
              <div className="space-y-2">
                {data.threat_actor_attribution.length === 0 ? (
                  ...
                  data.threat_actor_attribution.map((actor, idx) => (
                    <div key={idx} className="rounded-md border border-slate-700 bg-slate-800/70 p-3 text-sm">
                  ...
                  <div key={iocType}>
                    ...
                    {iocs.length === 0 ? (
                      ...
                      <ul className="space-y-1 text-slate-200">
                        {iocs.map((ioc, idx) => (
                          <li key={idx} className="rounded border border-slate-700 bg-slate-800/60 px-2 py-1">
                            {String(ioc.value)}
                          </li>
                        ))}
                      </ul>
